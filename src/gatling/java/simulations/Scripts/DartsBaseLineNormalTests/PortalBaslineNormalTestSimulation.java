...
public class PortalBaslineNormalTestSimulation extends Simulation {   
    ...
    private static final String SMOKE_TEST_TWO_COURT_MANAGER_USERS = "Baseline Normal - DARTS - Portal - Court Manager Users";
    private static final String SMOKE_TEST_TWO_TRANSCRIBER_USERS = "Baseline Normal - DARTS - Portal - Transcriber Users";
    private static final String SMOKE_TEST_TWO_LANGUAGE_USERS = "Baseline Normal - DARTS - Portal - Language Shop Users";
    ...
    public PortalBaslineNormalTestSimulation() {
    ...
    private void setUpScenarios(HttpProtocolBuilder httpProtocolExternal, HttpProtocolBuilder httpProtocolInternal) {
        ...
        setUp(
            smokeJudgeUsers.injectOpen(
                rampUsers(AppConfig.JUDGE_RAMP_UP_USERS_NORMAL).during(Duration.ofMinutes(AppConfig.RAMP_UP_DURATION_OF_JUDGES)) 
            ).protocols(httpProtocolInternal),
            smokeCourtClerkUsers.injectOpen(
                rampUsers(AppConfig.COURT_CLERK_RAMP_UP_USERS_NORMAL).during(Duration.ofMinutes(AppConfig.RAMP_UP_DURATION_OF_COURT_CLERK)) 
            ).protocols(httpProtocolInternal),
            smokeCourtManagerUsers.injectOpen(
                rampUsers(AppConfig.COURT_MANAGER_RAMP_UP_USERS_NORMAL).during(Duration.ofMinutes(AppConfig.RAMP_UP_DURATION_OF_COURT_MANAGER)) 
            ).protocols(httpProtocolInternal),
            smokeTranscriberUsers.injectOpen(
                rampUsers(AppConfig.TRANSCRIBER_RAMP_UP_USERS_NORMAL).during(Duration.ofMinutes(AppConfig.RAMP_UP_DURATION_OF_TRANSCRIBER))
            ).protocols(httpProtocolExternal),
            smokeLanguageShopUsers.injectOpen(
                rampUsers(AppConfig.LANGUAGE_SHOP_RAMP_UP_USERS_NORMAL).during(Duration.ofMinutes(AppConfig.RAMP_UP_DURATION_OF_LANGUAGE_SHOP)) 
            ).protocols(httpProtocolExternal)
        );
    }
