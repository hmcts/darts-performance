...
public class NightlyRunSimulation extends Simulation {
    ...
    //     CompletableFuture<String> hearingQueryFuture = CompletableFuture.supplyAsync(() -> SQLQueryProvider
    //     .loadSQL("hearing_query.sql"));
    //     CompletableFuture<String> test1QueryFuture = CompletableFuture.supplyAsync(() -> SQLQueryProvider.loadSQL
    //     ("test1_query.sql"));
    //     CompletableFuture<String> test2QueryFuture = CompletableFuture.supplyAsync(() -> SQLQueryProvider.loadSQL
    //     ("test2_query.sql"));
    //     CompletableFuture<String> test3QueryFuture = CompletableFuture.supplyAsync(() -> SQLQueryProvider.loadSQL
    //     ("test3_query.sql"));
    ...
    public NightlyRunSimulation() {
        ...
        HttpProtocolBuilder httpProtocolSoapAddDocument =
        HttpUtil.getHttpProtocol()
        ...
        setUpScenarios(httpProtocolSoap, httpProtocolSoapAddDocument, httpProtocolApi, httpProtocolExternal,
    ...
    private void setUpScenarios(HttpProtocolBuilder httpProtocolSoap, HttpProtocolBuilder httpProtocolSoapAddDocument,
                                HttpProtocolBuilder httpProtocolApi, HttpProtocolBuilder httpProtocolExternal,
        ...
        ScenarioBuilder saopScenarios = scenario("Soap Proxy Scenario")
            ...
            .on(
                exec(RegisterWithUsernameScenario.RegisterWithUsername(
                    ...
                    .on(exec(AddCaseUserScenario.addCaseUser(EnvironmentURL.DARTS_SOAP_VIQ_EXTERNAL_USERNAME.getUrl(),
                        EnvironmentURL.DARTS_SOAP_VIQ_EXTERNAL_PASSWORD.getUrl())))
                    .repeat(AppConfig.NIGHTLY_RUN_REPEATS)
                    .on(exec(
                        GetCasesUserScenario.GetCaseSOAPUser(EnvironmentURL.DARTS_SOAP_VIQ_EXTERNAL_USERNAME.getUrl(),
                            EnvironmentURL.DARTS_SOAP_VIQ_EXTERNAL_PASSWORD.getUrl())))
                    .repeat(AppConfig.NIGHTLY_RUN_REPEATS)
                ...
                exec(feed(Feeders.createCourtClerkUsers())) // Load court clerk user data
                    ...
                    .repeat(5).on( // Repeat 5 times, once for each cas_id and defendant name
                        exec(session -> {
                            ...
                            // Retrieve the cas_id and defendant name from the session and set them for use in the
                            // scenario
                            ...
                            session = session
                                ...
                                .set("defendantFirstName",
                                    defendantName); // Set the defendant name for #{defendantFirstName} usage
                            ...
                            .exec(
                                DartsPortalRequestAudioScenario.DartsPortalRequestAudioDownload()) // Request audio
                            // download
                ...
                exec(feed(Feeders.createTranscriberUsers())) // Load court clerk user data
                    ...
                    .repeat(5).on( // Repeat 5 times, once for each cas_id and defendant name
                        exec(session -> {
                            ...
                            // download
                            .exec(
                                TranscriberAttachFileAndDownloadAudioScenario.TranscriberAttachFileAndDownloadAudio()) // Add File to Transcription
                            .exec(DartsPortalDeleteAudioRequestScenario.DartsPortalDeleteAudioRequest())
                        // Delete a random Audio request
                    )
                    // .exec(DartsPortalPreviewAudioScenario.DartsPortalPreviewAudioScenario())
