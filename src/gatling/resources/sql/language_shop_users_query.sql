WITH UserDetails AS (SELECT ua.usr_id,
                     ...
                     WHERE ua.user_name LIKE '%PerfLanguageShop%'
                       AND urch.cth_id NOT IN (111, 153, 112, 154, 95, 55, 65, 114, 155, 152, 133, 129, 70, 136,
                                               113) -- Exclude specific cth_id values
),
     FilteredUserDetails AS (SELECT *
                             FROM UserDetails
                             WHERE courthouse_rn = 1),
     UserCases AS (SELECT fud.usr_id,
     ...
     RandomDefendant AS (SELECT uc.usr_id,
                                uc.cas_id,
                                d.defendant_name,
                         ...
                         WHERE d.cas_id IS NOT NULL)
SELECT fud.user_email_address,
       fud.Password,
       fud.user_name,
       fud.cth_id,
       fud.courthouse_name,
       ...
       COALESCE(
               CASE
                   WHEN POSITION(' ' IN rd1.defendant_name) > 0 THEN SUBSTRING(rd1.defendant_name FROM 1 FOR
                                                                               POSITION(' ' IN rd1.defendant_name) - 1)
       ...
       COALESCE(
               CASE
                   WHEN POSITION(' ' IN rd2.defendant_name) > 0 THEN SUBSTRING(rd2.defendant_name FROM 1 FOR
                                                                               POSITION(' ' IN rd2.defendant_name) - 1)
       ...
       COALESCE(
               CASE
                   WHEN POSITION(' ' IN rd3.defendant_name) > 0 THEN SUBSTRING(rd3.defendant_name FROM 1 FOR
                                                                               POSITION(' ' IN rd3.defendant_name) - 1)
       ...
       COALESCE(
               CASE
                   WHEN POSITION(' ' IN rd4.defendant_name) > 0 THEN SUBSTRING(rd4.defendant_name FROM 1 FOR
                                                                               POSITION(' ' IN rd4.defendant_name) - 1)
       ...
       COALESCE(
               CASE
                   WHEN POSITION(' ' IN rd5.defendant_name) > 0 THEN SUBSTRING(rd5.defendant_name FROM 1 FOR
                                                                               POSITION(' ' IN rd5.defendant_name) - 1)
